...
[AddComponentMenu("Weapons/WeaponRocketLauncher")]
public class WeaponRocketLauncher : WeaponBase
{
	const float TIME_TO_PREPARE_FOR_FIRE = 0.15f;
	...
	public override float PreparedForFireProgress
	{
		...
		{
			if (m_PrepareForFire)
				return Mathf.Approximately(TIME_TO_PREPARE_FOR_FIRE, 0) ? 1 : Mathf.Clamp(m_PrepareForFireCounter/TIME_TO_PREPARE_FOR_FIRE, 0, 1);
			else
				return -1;
		}
	}
