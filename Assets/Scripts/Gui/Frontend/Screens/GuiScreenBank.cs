...
[AddComponentMenu("GUI/Frontend/Screens/GuiScreenBank")]
public class GuiScreenBank : GuiScreen
{
	...
	// names of buttons
	readonly static string BUTTON_ADCOLONY = "ButtonAdColony";
	readonly static string BUTTON_BANNER = "ButtonBanner";
	...
	{
		static GuiPopupMessageBox m_Popup;
	...
	void VisitSite(SiteData site)
	{
		...
		if (site.m_FBSite) // Facebook
		{
			if ((site.m_Rewarded == false) && (FacebookPlugin.Instance.CurrentUser == null))
			{
				...
				FacebookPlugin.Instance.Login("user_likes", this.OnLoginResult);
			}
		}
		else // Twitter
		{
