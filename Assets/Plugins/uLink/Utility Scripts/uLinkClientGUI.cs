...
public static class uLinkCompatibility
{
...
public class uLinkClientGUI : uLink.MonoBehaviour
{
	...
	void OnGUI()
	{
		if (uLink.Network.lastError == uLink.NetworkConnectionError.NoError && uLink.Network.status == uLink.NetworkStatus.Connected && uLink.NetworkView.FindByOwner(uLink.Network.player).Length != 0 && (!lockCursor))
		{
			EnableGUI(false);
			return;
		}
		...
		else if (uLink.Network.status == uLink.NetworkStatus.Connected)
		{
			if (uLink.NetworkView.FindByOwner(uLink.Network.player).Length != 0)
			{
				if (lockCursor)
				{
					...
					if (Input.GetMouseButton(0)) uLinkCompatibility.Screen_lockCursor = true;
				}
			}
			else
			{
	...
	{
		if (lockCursor) uLinkCompatibility.Screen_lockCursor = !enabled;
